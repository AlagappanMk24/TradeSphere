...
{
    public class AccountRepository(UserManager<ApplicationUser> userManager, IConfiguration? configuration, IEmailService? emailService) : IAccountRepository
    {
        ...
        {
            var user = await userManager.FindByEmailAsync(currentEmail) ?? throw new Exception("UserNotFound");
        ...
        public async Task<ApplicationUser> UpdateProfileAsync(ApplicationUser ApplicationUser)
        {
            if (ApplicationUser is null) throw new Exception("User Not Found");
            var updateUser = await userManager.UpdateAsync(ApplicationUser);
            return ApplicationUser;
        }
        //Private Method
        private static string EmailBody(string confirmUrl)
        {
